...
import { Calendar as CalendarIcon, Clock, User, Mail, Phone, CheckCircle, ChevronRight, ChevronLeft } from "lucide-react";
...
export function BookingPage() {
  ...
  const [settings, setSettings] = useState<any>({ 
    ...
    workday_mon: 1, workday_tue: 1, workday_wed: 1, workday_thu: 1, 
    workday_fri: 1, workday_sat: 1, workday_sun: 1
  });
  ...
  const futureDates = useMemo(() => {
    ...
    const dayKeyMap = [
        "workday_sun", "workday_mon", "workday_tue", "workday_wed", 
        "workday_thu", "workday_fri", "workday_sat"
    ];
            ...
            if (preselectedEventId) {
                ...
                if (eventData) {
                    if (eventData.is_past) {
                        setError("Spiacenti, questo evento è già passato. Non è più possibile effettuare prenotazioni.");
                        setIsSettingsLoaded(true);
                        return;
                    }
                    if (eventData.is_full) {
                        setError("Questo evento è al completo. Non è più possibile accettare nuove prenotazioni online.");
                        setIsSettingsLoaded(true);
                        return;
                    }
                    ...
                    setSelectedSlot({
                        start: eDate.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' }),
                        end: eEnd.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })
                    });
                    setStep(3);
                }
            } else if (preselectedServiceId) {
                const s = servicesData.find((item: any) => item.id === preselectedServiceId || item.slug === preselectedServiceId);
                if (s) {
    ...
    if (selectedService && selectedDate) {
        async function loadSlots() {
            try {
                ...
                const data = await apiFetch(`/bookings/available-slots?service_id=${sId}&date=${selectedDate}`);
                setAvailableSlots(data.slots || []);
                if (data.message) setError(data.message);
            } catch (err: any) {
                setError(err.message || "Errore nel caricamento dei posti disponibili");
            } finally {
                setLoading(false);
            }
        }
  ...
  const handleBooking = async (e: React.FormEvent) => {
    e.preventDefault();
    if ((!selectedService && !selectedEvent) || !selectedDate || !selectedSlot) return;
    ...
    return (
        <div className="min-h-screen bg-brand-base flex items-center justify-center p-4">
            <div className="max-w-md w-full bg-white p-8 rounded-3xl shadow-xl text-center space-y-6 border border-brand-primary/10">
                ...
                <h1 className="text-3xl font-serif text-brand-primary">Prenotazione Confermata!</h1>
                <p className="text-brand-contrast/70">
                    Grazie {form.name}, la tua prenotazione per <strong>{selectedService?.title || selectedEvent?.title}</strong> è stata ricevuta. 
    ...
    <div className="min-h-screen bg-brand-base flex items-center justify-center py-20 font-sans">
        <div className="container mx-auto max-w-4xl px-4 relative z-10">
            <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-brand-primary/5">
                ...
                <div className="bg-brand-primary/5 p-6 border-b border-brand-primary/10">
                    <div className="flex justify-between items-center max-w-xs mx-auto">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step >= 1 ? 'bg-brand-primary text-white' : 'bg-gray-200 text-gray-500'}`}>1</div>
                        <div className={`h-0.5 w-10 ${step >= 2 ? 'bg-brand-primary' : 'bg-gray-200'}`}></div>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step >= 2 ? 'bg-brand-primary text-white' : 'bg-gray-200 text-gray-500'}`}>2</div>
                        <div className={`h-0.5 w-10 ${step >= 3 ? 'bg-brand-primary' : 'bg-gray-200'}`}></div>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step >= 3 ? 'bg-brand-primary text-white' : 'bg-gray-200 text-gray-500'}`}>3</div>
                    </div>
                ...
                <div className="p-8">
                    ...
                    {error && preselectedEventId && (
                        <div className="text-center py-12 space-y-6">
                            ...
                            <h2 className="text-2xl font-serif text-brand-primary">Evento al Completo</h2>
                            <p className="text-brand-contrast/70 max-w-md mx-auto">
                                {error} Puoi contattarci direttamente per essere inserito nella lista d'attesa in caso di rinunce.
                            </p>
                            ...
                            {services.map(s => (
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <h3 className="font-serif text-xl text-brand-primary group-hover:translate-x-1 transition-transform">{s.title}</h3>
                                        <ChevronRight size={20} className="text-brand-primary/40" />
                                    </div>
                    ...
                    {step === 2 && (
                        <div className="space-y-10">
                            {!isSettingsLoaded ? (
                                <div className="flex justify-center p-20"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-brand-primary"></div></div>
                            ) : (
                                <>
                                    <div>
                                        ...
                                        <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar -mx-2 px-2">
                                            {futureDates.map(d => (
                                                ...
                                                >
                                                    <span className="text-xs uppercase tracking-wider opacity-70">{d.day}</span>
                                                    <span className="text-2xl font-serif font-bold">{d.num}</span>
                                    ...
                                    <div className="flex justify-between pt-6 border-t border-brand-primary/10">
                                        <button onClick={() => setStep(1)} className="flex items-center gap-2 text-brand-contrast/60 hover:text-brand-primary font-medium">
                    ...
                    {/* STEP 3: CONTACT FORM */}
                    {step === 3 && (
                        <form onSubmit={handleBooking} className="space-y-8 max-w-md mx-auto">
                            <div className="bg-brand-primary/5 p-6 rounded-2xl border border-brand-primary/10 space-y-2">
                                <p className="text-sm font-bold uppercase tracking-widest text-brand-primary">Riepilogo</p>
                                <p className="text-xl font-serif text-brand-contrast">{selectedService?.title || selectedEvent?.title}</p>
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                >
                                    {loading ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> : "Conferma Prenotazione"}
                                </button>
                                <button 
                                    type="button"
                                    onClick={() => setStep(2)} 
            ...
            <p className="text-center mt-8 text-sm text-brand-contrast/40 px-8">
                Prenotando dichiari di accettare la nostra <a href="/privacy-policy" className="underline hover:text-brand-primary">Privacy Policy</a>. La cancellazione è gratuita fino a 48 ore prima.
            </p>
        </div>
    </div>
  );
