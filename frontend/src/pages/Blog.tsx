        ...
        <div className="container mx-auto px-4">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20 bg-white/50 rounded-2xl border border-brand-primary/10">
              <h3 className="text-2xl font-serif text-brand-primary mb-4">Nessun articolo pubblicato</h3>
              <p className="text-brand-contrast/70">Trona presto per leggere nuovi contenuti.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <article key={post.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-brand-primary/10 hover:shadow-md transition-all group flex flex-col h-full">
                  <Link to={`/blog/${post.slug || post.id}`} className="block relative aspect-[4/3] overflow-hidden bg-brand-primary/5">
                    {post.imageUrl ? (
                      <img 
                        src={post.imageUrl} 
                        alt={post.title}
                      ...
                      <div className="w-full h-full flex items-center justify-center text-brand-primary/20">
                        <span className="font-serif text-xl">Stefania Mastroianni</span>
                      </div>
                    )}
                  </Link>
                  ...
                  <div className="p-8 flex flex-col flex-grow">
                    ...
                    <h3 className="text-xl md:text-2xl font-serif text-brand-primary mb-4 line-clamp-2">
                      <Link to={`/blog/${post.slug || post.id}`} className="hover:text-brand-secondary transition-colors">
                        {post.title}
                      </Link>
                    </h3>
                    ...
                    <Link to={`/blog/${post.slug || post.id}`} className="inline-block mt-auto">
                      <Button variant="outline" className="w-full group-hover:bg-brand-primary group-hover:text-white transition-colors flex items-center justify-center">
                        Leggi l'articolo
