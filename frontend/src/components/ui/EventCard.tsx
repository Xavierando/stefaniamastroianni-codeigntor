...
export function EventCard({ event }: { event: EventProps }) {
  return (
    <motion.div
      ...
      className="w-full"
    >
      <Card className={`group flex flex-col bg-transparent border-0 shadow-none hover:shadow-soft transition-all duration-500 rounded-[2rem] overflow-hidden ${event.isPast ? 'opacity-70 grayscale-[0.3]' : ''}`}>
        ...
        <div className="relative aspect-[4/3] sm:aspect-[16/10] overflow-hidden rounded-[2rem]">
          ...
          <div className="absolute top-4 left-4">
            <span className="bg-white/90 text-[10px] font-bold uppercase tracking-widest text-brand-contrast px-4 py-2 rounded-full shadow-sm">
              {event.category}
            </span>
          </div>
        </div>
